...
To write the output, I employ an IReportWriter interface along with a ReportWriter abstract class containing some virtual abstract methods.  There are 2 concrete classes: ExcelWriter and TextWriter, and each has some overridden methods specific to its own implementation, but each also relies upon some common virtual methods defined in ReportWriter.
